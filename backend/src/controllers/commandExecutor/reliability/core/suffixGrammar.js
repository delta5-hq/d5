...
export const HISTORICAL_SUFFIX_SHAPES = Object.freeze([
  '[✓✗⚠]\\s+(\\d+\\/\\d+\\s+(best\\s+of\\s+\\d+(\\s+·\\s+[\\d.]+)?|first-survivor[^\\]]*|passed)|electd|elect\\s+failed)',
  '✓\\s+retry-\\d+',
  '✗\\s+\\d+\\s+attempts',
  '✗\\s+invalid',
...
export const ENGINE_SUFFIX_SHAPES = Object.freeze([
  '↻ \\d+/\\d+ ⚠',
  '↻ \\d+/\\d+',
  '✓ \\d+/\\d+ ⚠',
  '✓ \\d+/\\d+',
...
export const HISTORICAL_SUFFIX_RE = new RegExp(
  `\\s*\\[(?:${HISTORICAL_SUFFIX_SHAPES.map(s => `(?:${s})`).join('|')})\\]\\s*$`,
  'i',
)

export const ENGINE_SUFFIX_RE = new RegExp(`\\s*\\[(?:${ENGINE_SUFFIX_SHAPES.join('|')})\\]\\s*$`)
