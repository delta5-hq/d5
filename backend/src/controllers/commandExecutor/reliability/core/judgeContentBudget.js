...
const computeBudgetFromContextTokens = (nForks, contextTokens) => {
  if (nForks <= 0) return MAX_PER_FORK_CHARS

  const availableChars = Math.max(0, (contextTokens - PROMPT_OVERHEAD_TOKENS) * CHARS_PER_TOKEN)
...
export const computePerForkContentBudget = (nForks, configuredFamilies) => {
  ...
  return computeBudgetFromContextTokens(nForks, contextTokens)
}

export const computePerForkContentBudgetFromResolvedModels = (nForks, resolvedModels) => {
  const validContextWindows = (resolvedModels ?? [])
    .map(model => Number(model?.chunkSize))
    .filter(Number.isFinite)
    .filter(chunkSize => chunkSize > 0)

  if (validContextWindows.length === 0) {
    return computeBudgetFromContextTokens(nForks, FALLBACK_CONTEXT_TOKENS)
  }

  return computeBudgetFromContextTokens(nForks, Math.min(...validContextWindows))
}
