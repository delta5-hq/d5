...
const scopedLLMServices = [
  {
    service: "openai",
    userConfig: { apiKey: "user-scope-openai-key", model: "user-model" },
    workflowConfig: {
      apiKey: "workflow-scope-openai-key",
      model: "workflow-model",
    },
    visiblePath: "openai.model",
    userVisibleValue: "user-model",
    workflowVisibleValue: "workflow-model",
    encryptedSecretPath: "openai.apiKey",
  },
  {
    service: "custom_llm",
    userConfig: {
      apiRootUrl: "https://user.example.com",
      apiType: "OpenAI compatible",
      apiKey: "user-scope-custom-key",
      maxTokens: 1000,
      model: "user-custom-model",
    },
    workflowConfig: {
      apiRootUrl: "https://workflow.example.com",
      apiType: "OpenAI compatible",
      apiKey: "workflow-scope-custom-key",
      maxTokens: 2000,
      model: "workflow-custom-model",
    },
    visiblePath: "custom_llm.apiRootUrl",
    userVisibleValue: "https://user.example.com",
    workflowVisibleValue: "https://workflow.example.com",
    encryptedSecretPath: "custom_llm.apiKey",
    additionalVisiblePaths: [
      {
        path: "custom_llm.model",
        userValue: "user-custom-model",
        workflowValue: "workflow-custom-model",
      },
    ],
  },
];
...
describe("Integration Router", () => {
  ...
  describe("POST /integration/icons/download", () => {
    it("requires icon URL", async () => {
      const res = await subscriberRequest
        .post("/integration/icons/download")
        .send({});
  ...
  describe("POST /integration/midjourney/create", () => {
    it("requires Midjourney configuration", async () => {
      const res = await subscriberRequest
        .post("/integration/midjourney/create")
        .send({ prompt: "test" });
  ...
  describe("POST /integration/midjourney/upscale", () => {
    it("requires Midjourney configuration", async () => {
      const res = await subscriberRequest
        .post("/integration/midjourney/upscale")
        .send({ taskId: "test", index: 1 });
  ...
  describe("GET /integration/zoom/meetings/:id/recordings", () => {
    it("requires Zoom configuration", async () => {
      const res = await subscriberRequest.get(
        "/integration/zoom/meetings/test123/recordings",
      );
  ...
  describe("PUT /integration/:service/update", () => {
    ...
    it("updates Custom LLM configuration in mock mode without reaching external endpoints", async () => {
      const res = await subscriberRequest
        .put("/integration/custom_llm/update")
        .send({
      ...
      expect(res.body.custom_llm.model).toBe("test-model");

      const persistedRes = await subscriberRequest.get(
        "/integration/custom_llm",
      );
      expect(persistedRes.status).toBe(200);
      expect(persistedRes.body.custom_llm.model).toBe("test-model");
    });
    ...
    it.each([
      ...
      },
      {
        ...
        errorText: "endpoint is invalid",
      },
    ])(
      "does not persist Custom LLM configuration after $name validation failure",
      async ({ invalidConfig, errorText }) => {
        const persistedConfig = {
          apiRootUrl: "http://127.0.0.1:1",
          apiType: "OpenAI compatible",
        };

        const validRes = await subscriberRequest
          .put("/integration/custom_llm/update")
          .send(persistedConfig);
        expect(validRes.status).toBe(200);
        expect(validRes.body.custom_llm.apiRootUrl).toBe(
          persistedConfig.apiRootUrl,
        );
        expect(validRes.body.custom_llm.apiType).toBe(persistedConfig.apiType);

        const invalidRes = await subscriberRequest
          .put("/integration/custom_llm/update")
          .send(invalidConfig);
        expect(invalidRes.status).toBe(400);
        expect(invalidRes.body.message).toContain(errorText);
        expect(invalidRes.body).not.toHaveProperty("custom_llm");

        const persistedRes = await subscriberRequest.get(
          "/integration/custom_llm",
        );
        expect(persistedRes.status).toBe(200);
        expect(persistedRes.body.custom_llm.apiRootUrl).toBe(
          persistedConfig.apiRootUrl,
        );
        expect(persistedRes.body.custom_llm.apiType).toBe(
          persistedConfig.apiType,
        );
      },
    );
    ...
    it("creates new llmvectors document for deepseek when none exists", async () => {
      const res = await subscriberRequest
        .put("/integration/deepseek/update")
        .send({
    ...
    it("handles multiple services without store isolation violations", async () => {
      await subscriberRequest
        .put("/integration/deepseek/update")
        .send({ apiKey: "deepseek-key" });
      await subscriberRequest
        .put("/integration/openai/update")
        .send({ apiKey: "openai-key" });
      await subscriberRequest
        .put("/integration/claude/update")
  ...
  describe("workflow-scoped LLM service persistence", () => {
    let workflowId;

    beforeEach(async () => {
      const workflow = await testDataFactory.createWorkflow({
        title: "LLM Scope Isolation Workflow",
        userId: "subscriber",
      });
      workflowId = workflow.workflowId;
    });

    it.each(scopedLLMServices)(
      "$service keeps user and workflow scopes isolated on save and read",
      async (serviceCase) => {
        const {
          service,
          userConfig,
          ...
          visiblePath,
          userVisibleValue,
          workflowVisibleValue,
          encryptedSecretPath,
          additionalVisiblePaths = [],
        } = serviceCase;

        const userSave = await subscriberRequest
          .put(`/integration/${service}/update`)
          .send(userConfig);
        expect(userSave.status).toBe(200);

        const workflowSave = await subscriberRequest
          .put(`/integration/${service}/update?workflowId=${workflowId}`)
          .send(workflowConfig);
        expect(workflowSave.status).toBe(200);

        const userRead = await subscriberRequest.get("/integration");
        const workflowRead = await subscriberRequest.get(
          `/integration?workflowId=${workflowId}`,
        );

        expect(userRead.status).toBe(200);
        expect(workflowRead.status).toBe(200);
        expect(valueAtPath(userRead.body, visiblePath)).toBe(userVisibleValue);
        expect(valueAtPath(workflowRead.body, visiblePath)).toBe(
          workflowVisibleValue,
        );
        for (const assertion of additionalVisiblePaths) {
          expect(valueAtPath(userRead.body, assertion.path)).toBe(
            assertion.userValue,
          );
          expect(valueAtPath(workflowRead.body, assertion.path)).toBe(
            assertion.workflowValue,
          );
        }
        expect(
          valueAtPath(userRead.body.secretsMeta, encryptedSecretPath),
        ).toBe(true);
        expect(
          valueAtPath(workflowRead.body.secretsMeta, encryptedSecretPath),
        ).toBe(true);

        const userDoc = await rawIntegrationDoc("subscriber");
        const workflowDoc = await rawIntegrationDoc("subscriber", workflowId);

        expect(userDoc).toBeTruthy();
        expect(workflowDoc).toBeTruthy();
        expect(userDoc.workflowId).toBeNull();
        expect(workflowDoc.workflowId).toBe(workflowId);
        expect(valueAtPath(userDoc, encryptedSecretPath)).toBeTruthy();
        expect(valueAtPath(workflowDoc, encryptedSecretPath)).toBeTruthy();
        expect(valueAtPath(workflowDoc, encryptedSecretPath)).not.toBe(
          valueAtPath(userDoc, encryptedSecretPath),
        );
      },
    );

    it.each(scopedLLMServices)(
      "$service keeps workflow scope after user-scope deletion",
      async (serviceCase) => {
        const {
          service,
          userConfig,
        ...
        const deleteUser = await subscriberRequest.delete(
        ...
        expect(userRead.status).toBe(200);
        expect(workflowRead.status).toBe(200);
        expect(userRead.body[service]).toBeUndefined();
        expect(valueAtPath(workflowRead.body, visiblePath)).toBe(
          workflowVisibleValue,
        );
      },
...
describe("Integration Router - Administrator Tests", () => {
  ...
  describe("Administrator API Access", () => {
    ...
    it("should update integration settings", async () => {
      const res = await administratorRequest
        .put("/integration/openai/update")
        .send({ apiKey: "updated-admin-key" });
...
describe("Array Item Validation — args field contract for mcp and rpc", () => {
  ...
  const VALID_ARGS_CASES = [
    ...
    {
      label: "multiple string elements",
      payload: { args: ["server.js", "--flag", "--p=8"] },
    },
  ];
  ...
  it("rejected item leaves no trace in GET /integration — write is atomic", async () => {
    ...
    expect((getRes.body.mcp ?? []).map((item) => item.alias)).not.toContain(
      probe,
    );
  });

  it("sequence of invalid submissions does not corrupt GET /integration", async () => {
    const probes = INVALID_ARGS_CASES.map(({ args }, i) => ({
      alias: `/corruption-probe-${i}`,
      args,
    }));
  ...
  it("PUT update accepts partial MCP and RPC patches without create-only fields", async () => {
    ...
    for (const { field, alias, create, patch } of cases) {
      ...
      expect(getRes.status).toBe(200);
      expect(
        (getRes.body[field] ?? []).find((item) => item.alias === alias),
      ).toMatchObject(patch);
    }
  });
});
...
describe("Array Item Scope Isolation — workflowId segregation", () => {
  ...
  beforeAll(async () => {
    await testOrchestrator.prepareTestEnvironment();
    const wf = await testDataFactory.createWorkflow({
      title: "Scope Isolation Workflow",
      userId: "subscriber",
    });
  ...
  it("item added to workflow scope is absent from user-level GET", async () => {
    ...
    expect((userRes.body.mcp ?? []).map((m) => m.alias)).not.toContain(
      "/wf-only",
    );
  });
  ...
  it("workflow-scoped GET includes user-level items via fallback when no workflow doc exists", async () => {
    ...
    const freshWf = await testDataFactory.createWorkflow({
      title: "Fallback NoDoc Workflow",
      userId: "subscriber",
    });
    ...
    const wfRes = await subscriberRequest.get(
      `/integration?workflowId=${freshWf.workflowId}`,
    );
  ...
  it("two different workflow scopes do not share array items", async () => {
    const wf2 = await testDataFactory.createWorkflow({
      title: "Second Isolation Workflow",
      userId: "subscriber",
    });
    ...
    const res1 = await subscriberRequest.get(
      `/integration?workflowId=${workflowId}`,
    );
    const res2 = await subscriberRequest.get(
      `/integration?workflowId=${wf2.workflowId}`,
    );

    expect((res1.body.mcp ?? []).map((m) => m.alias)).toContain("/wf1-item");
    expect((res1.body.mcp ?? []).map((m) => m.alias)).not.toContain(
      "/wf2-item",
    );
    expect((res2.body.mcp ?? []).map((m) => m.alias)).toContain("/wf2-item");
    expect((res2.body.mcp ?? []).map((m) => m.alias)).not.toContain(
      "/wf1-item",
    );
  });

  it("DELETE of workflow-scoped item is idempotent for non-existent alias", async () => {
    const res = await subscriberRequest.delete(
      `/integration/mcp/items/${encodeURIComponent("/ghost")}?workflowId=${workflowId}`,
    );
  ...
  it("DELETE of user-scope item is idempotent for non-existent alias", async () => {
    const res = await subscriberRequest.delete(
      `/integration/mcp/items/${encodeURIComponent("/ghost")}`,
    );
    expect(res.status).toBe(204);
  });
  ...
  it("DELETE of workflow-scoped item does not affect same alias at user scope", async () => {
    ...
    await subscriberRequest.delete(
      `/integration/mcp/items/${encodeURIComponent("/shared-alias")}?workflowId=${workflowId}`,
    );

    const userRes = await subscriberRequest.get("/integration");
    expect(
      (userRes.body.mcp ?? []).find((m) => m.alias === "/shared-alias")
        ?.toolName,
    ).toBe("user");
  });
  ...
  it("after last workflow-scoped item deleted, GET with workflowId falls back to user-level", async () => {
    ...
    const freshWf = await testDataFactory.createWorkflow({
      title: "Fallback AfterDelete Workflow",
      userId: "subscriber",
    });
    ...
    await subscriberRequest.delete(
      `/integration/mcp/items/${encodeURIComponent("/wf-only-item")}?workflowId=${freshWf.workflowId}`,
    );

    const res = await subscriberRequest.get(
      `/integration?workflowId=${freshWf.workflowId}`,
    );
    expect((res.body.mcp ?? []).map((m) => m.alias)).toContain(
      "/fallback-anchor",
    );
    expect((res.body.mcp ?? []).map((m) => m.alias)).not.toContain(
      "/wf-only-item",
    );
  });
  ...
  it("empty workflowId query param is treated as user scope", async () => {
    ...
    const userRes = await subscriberRequest.get("/integration");
    expect((userRes.body.mcp ?? []).map((m) => m.alias)).toContain(
      "/empty-wf-id",
    );
  });
  ...
  it("RPC items obey scope isolation identically to MCP items", async () => {
    ...
    const userRes = await subscriberRequest.get("/integration");
    expect((userRes.body.rpc ?? []).map((r) => r.alias)).not.toContain(
      "/wf-rpc",
    );
