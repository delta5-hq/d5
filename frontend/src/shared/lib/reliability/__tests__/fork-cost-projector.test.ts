 ...
 *
 *   Sᵢ = count of executable nodes in /refine-i's immediate scope
 *        Parent node + all non-/refine descendants, accumulating commodity :n=N
 *        Nested /refine cells are scope BOUNDARIES — their subtrees are excluded
 ...
 *   Reliability cells (/refine, /validate): commodity :n=N is SUPPRESSED — always 1
 */
...
describe('projectForkCost — guard conditions', () => {
  ...
  it('returns 0 when refine node has no parent field', () => {
    const n = nodes({ r: { id: 'r', command: '/refine :n=2', children: [] } })
    expect(projectForkCost(n.r, n)).toBe(0)
  })

  it('returns 0 when parent id is not present in nodes', () => {
    const n = nodes({ r: { id: 'r', parent: 'ghost', command: '/refine :n=3', children: [] } })
    expect(projectForkCost(n.r, n)).toBe(0)
  })

  it('returns 0 for undefined refine node', () => {
    expect(projectForkCost(undefined, nodes())).toBe(0)
  })
})

describe('projectForkCost — flat scope (parent + siblings)', () => {
  it('refine is the only child: cost = N × 1 (parent alone)', () => {
    /*
     * parent   ← 1
     *   └── /refine :n=5   ← excluded
     * scope = 1, cost = 5 × 1 = 5
    ...
    const n = nodes({
      parent: { id: 'parent', children: ['r'] },
      r: { id: 'r', parent: 'parent', command: '/refine :n=5', children: [] },
    })
  ...
  it('a /refine sibling is a scope boundary — excluded from count', () => {
    /*
     * parent                  ← 1
     *   ├── /refine :n=2 (other)  ← excluded (boundary)
     *   └── /refine :n=5 (main)   ← excluded (self)
     * scope = 1, cost = 5 × 1 = 5
     */
    const n = nodes({
      parent: { id: 'parent', children: ['other', 'r'] },
      other: { id: 'other', parent: 'parent', command: '/refine :n=2', children: [] },
      r: { id: 'r', parent: 'parent', command: '/refine :n=5', children: [] },
    })
    expect(projectForkCost(n.r, n)).toBe(5)
  })
...
describe('projectForkCost — recursive scope (siblings with subtrees)', () => {
  it('sibling subtree is walked recursively up to a nested /refine boundary', () => {
    /*
     * parent (/steps)       ← 1
     *   ├── step1 (/chat)   ← 2
     *   │     └── sum (/summarize) ← 3
  ...
  it('nested /refine inside sibling is a scope boundary: children beyond it excluded from outer scope', () => {
    ...
    const n = nodes({
      ...
      innerR: { id: 'innerR', parent: 'mid', command: '/refine :n=2', children: ['c1', 'c2'] },
...
describe('projectForkCost — nested /refine produces additive cost, not multiplicative', () => {
  ...
  it('three-level nesting: Σ Nᵢ × Sᵢ across all levels', () => {
     ...
     *   │     │     └── l3R (/refine :n=4)  cost: 4×1 = 4
     *   │     └── l2R (/refine :n=3)        cost: 3×2 + 4 = 10
     *   └── l1R (/refine :n=2)              cost: 2×3 + 10 = 16
     */
    const n = nodes({
  ...
  it('two sibling nested /refine: both inner costs added once', () => {
    /*
     ...
     *   └── outerR (/refine :n=5)         cost: 5×3 + 2 + 3 = 20
     */
    const n = nodes({
...
describe('projectForkCost — commodity :n=N on plain LLM cells', () => {
  ...
  it(':limit= on /refine is ignored by the cost projector', () => {
    /*
     * parent (/chat :n=10)          ← counts as 10
     *   └── /refine :n=3 :limit=xs  ← :limit= is a refusal threshold, not a cost modifier
