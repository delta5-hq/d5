...
test.describe('COMMODITY_OUTCOME_SUFFIX_RE — fraction reporting for commodity/refine nodes', () => {
  ...
  const rejectCases: [string, string][] = [
    ...
    ['validate all retries exhausted',    'Task [✗ 3×]'],
    ['validate invalid criterion',        'Task [✗ !]'],
    ['refine fallback (⚠ symbol)',         'Task [⚠ 0/3]'],
  ]
...
test.describe('REFINE_FALLBACK_SUFFIX_RE — refine committed a fallback winner when all forks failed', () => {
  test('matches [⚠ 0/3] — zero of N forks met criteria, fallback committed', () => {
    expect('Task [⚠ 0/3]').toMatch(REFINE_FALLBACK_SUFFIX_RE)
  })

  test('matches single-fork fallback [⚠ 0/1]', () => {
    expect('Task [⚠ 0/1]').toMatch(REFINE_FALLBACK_SUFFIX_RE)
  })

  test('does not match no-judge-signal form [⚠ ∅]', () => {
    expect('Task [⚠ ∅]').not.toMatch(REFINE_FALLBACK_SUFFIX_RE)
  })

  test('does not match a non-zero eligible fraction — only 0/N is a fallback', () => {
    expect('Task [⚠ 1/3]').not.toMatch(REFINE_FALLBACK_SUFFIX_RE)
  })

  test('does not match commodity success [✓ 2/3]', () => {
    expect('Task [✓ 2/3]').not.toMatch(REFINE_FALLBACK_SUFFIX_RE)
  })
})
