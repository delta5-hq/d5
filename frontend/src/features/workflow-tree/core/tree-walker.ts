...
export function* createTreeWalker(treeData: FlatTreeData, refresh: boolean) {
  ...
  while (stack.length > 0) {
    ...
    const { node, depth, ancestorContinuation, hasMoreSiblings, parentRowIndex, parentSparkDelay, ancestorGenerated } =
      entry
    ...
    // isPrompt stays strict — structural membership in the parent's prompts — so consumers like the
    // lazy-split preview keep their exact hierarchy. isGenerated is the visual generated state and
    // propagates into a subtree: fan-out registers a cloned `/steps` container in its parent's prompts
    // but not the container's own cloned children, so they render generated by inheriting from it.
    const isPrompt = isPromptNode(node.id, nodes)
    const isGenerated = ancestorGenerated || isPrompt
