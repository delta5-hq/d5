...
export function bindExecuteAction(store: Store<WorkflowStoreState>, persister: DebouncedPersister): ExecutionActions {
  ...
  const executeCommand = async (node: NodeData, queryType: string): Promise<boolean> => {
    ...
    try {
      ...
      // Only nodes newly generated by THIS execution may stand in as clipboard until their spark
      // arrives. A reveal target that already existed (an operator node, or one generated by an
      // earlier execution) keeps its command presentation - its result still reveals on spark, but the
      // node itself must not flip to the unassigned clipboard state.
