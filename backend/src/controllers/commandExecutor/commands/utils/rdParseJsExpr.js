import {Ignore, All, Any, Optional, Star, Node} from './rdParse'
...
const TemplateInlineExpression = Node(All('${', IgnoreWhitespace(Expression), '}'), ([expression]) => [
  'expressions',
  expression,
])

const TemplateLiteral = Node(
  Ignore(null, All('`', Star(Any(InterpolationChunk, TemplateInlineExpression)), '`')),
...
// Object literal
const ShortNotation = Node(
  Identifier,
  withSrcMap(([expr]) => ({...expr, shortNotation: true})),
)
...
const PropertyDefinition = Node(
  Any(All(PropertyName, ':', Expression), ShortNotation, SpreadElement),
...
const PrimaryExpression = Node(
  Any(Literal, Identifier, ArrayLiteral, ObjectLiteral, All('(', CompoundExpression, ')')),
  withSrcMap(),
)
...
const MemberExpression = Node(
  All(Optional(/^new\b/), PrimaryExpression, Star(PropertyAccessOrArguments)),
  (parts, $, $last) => {
...
const UnaryExpression = Node(All(Star(UnaryOperator), MemberExpression), (parts, _, $next) =>
  parts.reduceRight((argument, {$, operator}) => srcMap({type: 'UnaryExpression', argument, operator}, $, $next)),
)
...
const BoundName = Node(
  IdentifierToken,
  withSrcMap(([name]) => ({bindingType: 'SingleName', name})),
)
const RestElement = Node(All('...', BoundName), ([rest]) => ({rest}))

const BindingList = (List, bindingType) =>
  Node(
    Any(
      All(
        Node(List, bound => ({bound})),
        Optional(All(',', Optional(RestElement))),
      ),
      Optional(RestElement),
    ),
    parts => parts.reduce((acc, part) => Object.assign(acc, part), {bound: [], bindingType}),
  )

const WithInitializer = Pattern =>
  Node(All(Pattern, Optional(All('=', Expression))), ([pattern, initializer]) =>
    initializer ? {pattern, initializer} : {pattern},
  )

const SingleNameBinding = WithInitializer(BoundName)

const BindingProperty = Node(Any(All(PropertyName, ':', BindingElement), SingleNameBinding), ([prop, pattern]) =>
  pattern ? {prop, ...pattern} : {prop: prop.pattern.name, ...prop},
)
