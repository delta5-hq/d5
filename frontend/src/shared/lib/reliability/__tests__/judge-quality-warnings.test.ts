...
function makeNodes(
  ...
  const nodes: Record<NodeId, NodeData> = {
    ...
    [nodeId]: { id: nodeId, title: 'elect', command: '', parent: parentId, children: [] } as NodeData,
  }
...
describe('computePreExecuteWarnings', () => {
  ...
  describe('guard conditions — commands that produce no warnings', () => {
    ...
    it('returns empty for a non-elect/non-validate command', () => {
      const nodes = makeNodes(nodeId, parentId)
      expect(computePreExecuteWarnings('/chat :n=3 query', nodeId, nodes, openaiOnly)).toEqual([])
    })
    it('returns empty for /elect with no valid :n= parameter', () => {
      const nodes = makeNodes(nodeId, parentId)
      expect(computePreExecuteWarnings('/elect without-n-param', nodeId, nodes, openaiOnly)).toEqual([])
    })
  })

  describe('/elect :n=N — singleProvider warning', () => {
    it('emits when no providers are configured', () => {
      const nodes = makeNodes(nodeId, parentId)
      expect(computePreExecuteWarnings('/elect :n=3', nodeId, nodes, noSettings)).toContainEqual({
    ...
    it('emits when exactly one family is configured', () => {
      const nodes = makeNodes(nodeId, parentId)
      expect(computePreExecuteWarnings('/elect :n=3', nodeId, nodes, openaiOnly)).toContainEqual({
        condition: 'singleProvider',
        severity: 'high',
      })
    })
    it('does not emit when two or more families are configured', () => {
      const nodes = makeNodes(nodeId, parentId)
      const warnings = computePreExecuteWarnings('/elect :n=3', nodeId, nodes, twoFamilies)
      expect(warnings.every(w => w.condition !== 'singleProvider')).toBe(true)
    })
  })

  describe('/elect :n=N — lowestTierOnly warning', () => {
    it('emits when all configured families are lower-tier and there are two or more', () => {
      const nodes = makeNodes(nodeId, parentId)
      expect(computePreExecuteWarnings('/elect :n=3', nodeId, nodes, yandexAndCustom)).toContainEqual({
    ...
    it('emits singleProvider instead of lowestTierOnly when there is exactly one lower-tier family', () => {
      const nodes = makeNodes(nodeId, parentId)
      const warnings = computePreExecuteWarnings('/elect :n=3', nodeId, nodes, yandexOnly)
    ...
    it('does not emit when at least one higher-tier family is present alongside lower-tier', () => {
      const nodes = makeNodes(nodeId, parentId)
      const warnings = computePreExecuteWarnings('/elect :n=3', nodeId, nodes, openaiAndYandex)
      expect(warnings.every(w => w.condition !== 'lowestTierOnly')).toBe(true)
    })
  ...
  describe('/elect :n=N — noReasoningMode warning', () => {
    it('emits when only higher-tier non-reasoning families are configured', () => {
      const nodes = makeNodes(nodeId, parentId)
      expect(computePreExecuteWarnings('/elect :n=3', nodeId, nodes, deepseekAndQwen)).toContainEqual({
    ...
    it('emits alongside singleProvider when exactly one non-reasoning family is configured', () => {
      const nodes = makeNodes(nodeId, parentId)
      const warnings = computePreExecuteWarnings('/elect :n=3', nodeId, nodes, deepseekOnly)
      expect(warnings).toContainEqual({ condition: 'singleProvider', severity: 'high' })
    ...
    it('emits alongside lowestTierOnly when all configured families are lowest-tier', () => {
      const nodes = makeNodes(nodeId, parentId)
      const warnings = computePreExecuteWarnings('/elect :n=3', nodeId, nodes, yandexAndCustom)
    ...
    it('does not emit when OpenAI is configured', () => {
      const nodes = makeNodes(nodeId, parentId)
      const warnings = computePreExecuteWarnings('/elect :n=3', nodeId, nodes, openaiOnly)
      expect(warnings.every(w => w.condition !== 'noReasoningMode')).toBe(true)
    })

    it('does not emit when Claude is configured', () => {
      const nodes = makeNodes(nodeId, parentId)
      const warnings = computePreExecuteWarnings('/elect :n=3', nodeId, nodes, claudeOnly)
      expect(warnings.every(w => w.condition !== 'noReasoningMode')).toBe(true)
    })
    ...
    it('does not emit when no providers are configured', () => {
      const nodes = makeNodes(nodeId, parentId)
      const warnings = computePreExecuteWarnings('/elect :n=3', nodeId, nodes, noSettings)
      expect(warnings.every(w => w.condition !== 'noReasoningMode')).toBe(true)
    })
  })

  describe('/elect :n=N — fallbackWithWeakJudge warning', () => {
    it('emits when :fallback is set and only one family is configured', () => {
      const nodes = makeNodes(nodeId, parentId)
      expect(computePreExecuteWarnings('/elect :n=3 :fallback', nodeId, nodes, openaiOnly)).toContainEqual({
    ...
    it('emits when :fallback is set and all configured families are lower-tier', () => {
      const nodes = makeNodes(nodeId, parentId)
      expect(computePreExecuteWarnings('/elect :n=3 :fallback', nodeId, nodes, yandexAndCustom)).toContainEqual({
        condition: 'fallbackWithWeakJudge',
        severity: 'high',
      })
    })
    ...
    it('does not emit when :fallback is set and two or more strong families are configured', () => {
      const nodes = makeNodes(nodeId, parentId)
      const warnings = computePreExecuteWarnings('/elect :n=3 :fallback', nodeId, nodes, twoFamilies)
      expect(warnings.every(w => w.condition !== 'fallbackWithWeakJudge')).toBe(true)
    })
    it('does not emit when :fallback is set and a higher-tier family is present alongside lower-tier', () => {
      const nodes = makeNodes(nodeId, parentId)
      const warnings = computePreExecuteWarnings('/elect :n=3 :fallback', nodeId, nodes, openaiAndYandex)
      expect(warnings.every(w => w.condition !== 'fallbackWithWeakJudge')).toBe(true)
    })
  })

  describe('/elect :n=N — juryDuplicates from sibling /validate :n=Nx', () => {
    it('emits when a sibling jury size exceeds the configured family count', () => {
      ...
      expect(computePreExecuteWarnings('/elect :n=2', nodeId, nodes, threeFamilies)).toContainEqual({
    ...
    it('does not emit when sibling jury size equals the configured family count', () => {
      ...
      const warnings = computePreExecuteWarnings('/elect :n=2', nodeId, nodes, threeFamilies)
    ...
    it('does not emit when sibling /validate has no :n= parameter', () => {
      ...
      const warnings = computePreExecuteWarnings('/elect :n=2', nodeId, nodes, openaiOnly)
      expect(warnings.every(w => w.condition !== 'juryDuplicates')).toBe(true)
    })
  ...
  describe('/validate — judge-selection warnings do not apply', () => {
    ...
    it('does not emit fallbackWithWeakJudge for /validate (that flag is only meaningful on /elect)', () => {
      const nodes = makeNodes(nodeId, parentId)
      const warnings = computePreExecuteWarnings('/validate :n=2 criterion', nodeId, nodes, openaiOnly)
      expect(warnings.every(w => w.condition !== 'fallbackWithWeakJudge')).toBe(true)
    })
